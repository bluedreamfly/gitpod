...
func main() {
	...
	http.HandleFunc("/status", func(w http.ResponseWriter, r *http.Request) {
		...
		hash := fmt.Sprintf("idePath=%s&projectPath=%s&host=%s&port=22&user=%s&type=ssh&deploy=false", url.QueryEscape("/ide-desktop/backend"), url.QueryEscape(wsInfo.CheckoutLocation), "35.205.211.83", url.QueryEscape(wsInfo.WorkspaceId))
		fmt.Println(hash)
		link := url.URL{
