...
internal class GitpodAuthService : OAuthServiceBase<Credentials>() {
    ...
    private class GitpodAuthRequest : OAuthRequest<Credentials> {
        ...
        override val authorizationCodeUrl = newFromEncoded("http://127.0.0.1:$port/${RestService.PREFIX}/$SERVICE_NAME/authorization_code")
        ...
        constructor(gitpodHost: String) {
            ...
            credentialsAcquirer = GitpodAuthCredentialsAcquirer(serviceUrl.resolve("token"), mapOf(
                "grant_type" to "authorization_code",
                "client_id" to CLIENT_ID,
                "redirect_uri" to authorizationCodeUrl.toExternalForm(),
                "code_verifier" to codeVerifier
            ))
            authUrlWithParameters = serviceUrl.resolve("authorize").addParameters(mapOf(
                "client_id" to CLIENT_ID,
                "redirect_uri" to authorizationCodeUrl.toExternalForm(),
