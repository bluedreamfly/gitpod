...
func (s *Server) ChannelForward(session *Session, newChannel ssh.NewChannel) {
	...
	clientConfig := &ssh.ClientConfig{
		...
		Auth: []ssh.AuthMethod{
			ssh.PublicKeysCallback(func() (signers []ssh.Signer, err error) {
				return []ssh.Signer{session.Remote.AuthKey}, nil
			}),
		},
	...
	conn, err := net.Dial("tcp", session.Remote.Address)
	if err != nil {
	...
	clientConn, clientChans, clientReqs, err := ssh.NewClientConn(conn, session.Remote.Address, clientConfig)
	if err != nil {
